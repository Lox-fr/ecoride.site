...
class UsersProvider
{
    ...
    private const ADMIN_PSEUDO = 'José';
    private const ADMIN_EMAIL = 'admin@ecoride.site';
    private const ADMIN_NAME = 'Studi';
    private const ADMIN_PHOTO_FILE_NAME = '000-admin-jose';

    private const EMPLOYEES_PSEUDOS = [
        'Mathilde', 'Emma', 'Axel', 'Esther', 'Cathy', 'Thomas', 'Yaël',
    ];
    private const EMPLOYEES_EMAILS = [
        'mathilde@ecoride.site', 'emma@ecoride.site', 'axel@ecoride.site', 'esther@ecoride.site', 'cathy@ecoride.site', 'thomas@ecoride.site', 'yael@ecoride.site',
    ];
    private const EMPLOYEES_NAMES = [
        'Séraphin', 'Beauchamp', 'Fauconnier', 'Vaillant', 'Delaunay', 'Tavernier', 'Briand',
    ];
    private const EMPLOYEES_PHOTO_FILE_NAMES = [
        '000-employee-mathilde', '000-employee-emma', '000-employee-axel', '000-employee-esther', '000-employee-cathy', '000-employee-thomas', '000-employee-yael',
    ];
    ...
    /**
     * Admin data used in SQL fixtures, returned as a User instance.
     *
     * The admin is created with the following data: pseudo, email, password, createdAt, active status,
    ...
    public function getAdmin(): User
    {
        $admin = new User();
        $admin
            ->setPseudo(self::ADMIN_PSEUDO)
            ->setEmail(self::ADMIN_EMAIL)
            ->setPassword($this->fakerFactory->password())
            ->setCreatedAt(new \DateTimeImmutable())
            ->setActive(true)
            ->setFirstName(self::ADMIN_PSEUDO)
            ->setLastName(self::ADMIN_NAME)
            ->setAddress($this->fakerFactory->address())
            ->setPhoneNumber($this->fakerFactory->regexify('0[67][0-9]{8}'))
            ->setPhotoFilename(self::ADMIN_PHOTO_FILE_NAME.'.jpg')
            ->setDateOfBirth(\DateTimeImmutable::createFromFormat(
                'Y-m-d', $this->fakerFactory->dateTimeBetween('-36 years', '-32 years')->format('Y-m-d')))
            ->setPetsAllowed($this->fakerFactory->boolean())
            ->setSmokersAllowed($this->fakerFactory->boolean())
            ->setDriverRoleChosen(true)
            ->setCredits($this->fakerFactory->numberBetween(1500, 5000));

        return $admin;
    }

    /**
     * Employees data used in SQL fixtures, returned as a collection of User instances.
     *
     * For each employee, the following data is provided: pseudo, email, password, createdAt, active status,
     * first name, last name, address, phone number, photo file name, date of birth, pets allowed, smokers allowed,
     * sum of ratings, number of ratings, and credits.
     *
     * Currently return 7 employees.
     *
     * @return ArrayCollection<int, User>
     */
    public function getCollectionOfEmployees(): ArrayCollection
    {
        $employeesCollection = new ArrayCollection();
        for ($i = 0; $i < \count(self::EMPLOYEES_PSEUDOS); ++$i) {
            $newEmployee = new User();
            $newEmployee
                ->setPseudo(self::EMPLOYEES_PSEUDOS[$i])
                ->setEmail(self::EMPLOYEES_EMAILS[$i])
                ->setPassword($this->fakerFactory->password())
                ->setCreatedAt(new \DateTimeImmutable())
                ->setActive(true)
                ->setFirstName(self::EMPLOYEES_PSEUDOS[$i])
                ->setLastName(self::EMPLOYEES_NAMES[$i])
                ->setAddress($this->fakerFactory->address())
                ->setPhoneNumber($this->fakerFactory->regexify('0[67][0-9]{8}'))
                ->setPhotoFilename(self::EMPLOYEES_PHOTO_FILE_NAMES[$i].'.jpg')
                ->setDateOfBirth(\DateTimeImmutable::createFromFormat(
                    'Y-m-d', $this->fakerFactory->dateTimeBetween('-45 years', '-21 years')->format('Y-m-d')))
                ->setPetsAllowed($this->fakerFactory->boolean())
                ->setSmokersAllowed($this->fakerFactory->boolean())
                ->setDriverRoleChosen(true)
                ->setCredits($this->fakerFactory->numberBetween(500, 1500));
